...
class SimpleBSGen(nn.Module):
    def __init__(self, facial_dims=51, audio_f=128, hidden_size=256, n_layer=4, dropout_prob=0.3):
        super(SimpleBSGen, self).__init__()
        self.audio_f = audio_f
        self.facial_dims = facial_dims
        ...
        self.lstm = nn.LSTM(self.in_size, self.hidden_size, num_layers=self.n_layer, batch_first=True,
                            bidirectional=True, dropout=self.dropout_prob)
        self.out = nn.Sequential(
            nn.Linear(self.hidden_size, self.hidden_size//2),
            nn.LeakyReLU(True),
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        for it, (in_audio, facial, in_id) in enumerate(train_loader):
            ...
            facial = facial.cuda()
            pre_frames = 4
            in_pre_face = facial.new_zeros(
                (facial.shape[0], facial.shape[1], facial.shape[2] + 1)).cuda()
